...
class CellsChallenge(_BaseDataset):
    ...
    def __init__(self, config=None):
        ...
        self.class_name_to_class_id = {'cell': '1', 'ignore': '10'}

        # Get sequences to eval and check gt files exist
        # self.seq_list, self.seq_lengths = self._get_seq_info()

        self.seq_list = sorted([dir for dir in os.listdir(self.config['GT_FOLDER']) if bool(re.search('\d\d$',dir))])
        self.seq_lengths = {}
        for seq in self.seq_list:
            file = self.config['GT_FOLDER']  + '/' + seq + '/gt/gt.txt'

            with open(file,'r') as f:
                lines = f.readlines()
                last_line = lines[-1].strip()
                self.seq_lengths[seq] = int(re.findall('\d+',last_line)[0])
                ...
                for seq in self.seq_list:
                    ...
                    if not os.path.isfile(curr_file):
                        ...
                        raise TrackEvalException(
                            'Tracker file not found: ' + tracker 
                            )
    ...
    def _get_seq_info(self):
        seq_list = os.listdir(self.config['TRACKERS_FOLDER'])
        seq_lengths = {}
